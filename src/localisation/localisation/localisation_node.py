...
class Localisation_Node(Node):
    def __init__(self):
        ...
        self.wheel_vel_noise = np.array([0.05, 0.05]) # Q matrix tuning
    ...
    def imu_callback(self, msg):
        ...
        z = -self.euler_from_quaternion(msg.orientation) # IMU mounted sideways, so subtract 90 degrees
    ...
    def broadcast_and_publish(self, time):
        ...
        cov = np.zeros((6, 6))
        cov[0:2, 0:2] = self.P[0:2, 0:2]    # x-y block (variance + cross-cov)
        cov[0, 5] = self.P[0, 2]             # x-yaw cross-covariance
        cov[1, 5] = self.P[1, 2]             # y-yaw cross-covariance
        cov[5, 0] = self.P[2, 0]             # yaw-x cross-covariance (symmetric)
        cov[5, 1] = self.P[2, 1]             # yaw-y cross-covariance (symmetric)
        cov[5, 5] = self.P[2, 2]             # yaw variance
