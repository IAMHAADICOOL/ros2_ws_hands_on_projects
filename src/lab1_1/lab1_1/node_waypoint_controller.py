...
class WaypointController(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('fallback_plan_topic', '/path')
        ...
        self.fallback_plan_topic = self.get_parameter('fallback_plan_topic').value
        ...
        if self.fallback_plan_topic and self.fallback_plan_topic != self.plan_topic:
            self.create_subscription(Path, self.fallback_plan_topic, self.plan_callback, 10)
        ...
        self.get_logger().info(
            f"Listening for plans on '{self.plan_topic}'"
            + (f" and '{self.fallback_plan_topic}'" if self.fallback_plan_topic and self.fallback_plan_topic != self.plan_topic else "")
        )
