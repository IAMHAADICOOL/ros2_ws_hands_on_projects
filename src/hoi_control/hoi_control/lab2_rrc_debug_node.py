...
class Lab2RRCDebugNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('target_x',  0.00)   # East
        self.declare_parameter('target_y',  0.159)  # North — same as rest
        self.declare_parameter('target_z',  0.40)   # Up    — 80mm lower than rest
        ...
        self.declare_parameter('method',    2)      # 0=transpose,1=pinv,2=DLS
    ...
    def _control_loop(self):
        ...
        if ee_world_enu is not None and self._j1_world_enu is not None:
            ...
            if self._loop_count % 30 == 0:
                self.get_logger().info(
                    ...
                    f'  r_tf={r_tf*1000:.1f}mm  z_tf={z_tf*1000:.1f}mm  '   # ADD z_tf HERE
                    f'r_fk: {(L2*np.cos(self.arm.q[1])+L3*np.cos(self.arm.q[1]+self.arm.q[2])+L4)*1000:.1f}mm')
        ...
        if ena:
            ...
            dq_arr = dq.flatten()
            q      = self.arm.q
            if q[1] <= -1.40 and dq_arr[1] < 0: dq_arr[1] = 0.0
            if q[1] >=  0.00 and dq_arr[1] > 0: dq_arr[1] = 0.0
            if q[2] <= -1.20 and dq_arr[2] < 0: dq_arr[2] = 0.0
            if q[2] >=  0.60 and dq_arr[2] > 0: dq_arr[2] = 0.0
